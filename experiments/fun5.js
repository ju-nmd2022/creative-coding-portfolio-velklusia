...
function draw() {
    ...
    for (let y = 0; y < rows; y++) {
        ...
        for (let x = 0; x < cols; x++) {
            ...
            let z = map(dist, 0, sqrt(sq(width / 2) + sq(height / 2)), 200, 0); // Changed the range for more intensity
    ...
    for (let x = 0; x < cols; x++) {
        ...
        for (let y = 0; y < rows; y++) {
            ...
            let xDistort = xOffset + cos(radians(dist * 0.5) * 2 + time * 2) * (50 + z * 0.1);
            let yDistort = yOffset + sin(radians(dist * 0.5) * 2 + time * 2) * (50 + z * 0.1);
